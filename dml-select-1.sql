...
select *from event;
select event_no,clack_in_seconds
from event
order by event_no;
...
select event_no
from event
where raid_points + defending_points >=80
order by event_no;
...
-- 8. **Write a query to display all the stadium names that ends with `Complex`. Display the records sorted in ascending order based on stadium namesselect *from venue;
select stadium_name
    from venue
...
-- 10. **Write a query to display the game details of those games that were played between January and March of 2020. Display the records sorted in descending order based on idselect id
    from game
